...
static uint8_t XPT2046_ReadAdc_Smooth_XY ( strType_XPT2046_Coordinate * pScreenCoordinate )
{
	...
	//while( XPT2046_PENIRQ_Read() == XPT2046_PENIRQ_ActiveLevel); //如果一直按下，等待释放。否则会一直跳变
...
void Calibrate_or_Get_TouchParaWithFlash(uint8_t LCD_Mode,uint8_t forceCal)
{
	...
	W25Q128_Init();
	//读取触摸参数标志
	W25Q128_Read(&para_flag,FLASH_TOUCH_PARA_FLAG_ADDR,1);
	...
	{ 		
		...
		if( para_flag == FLASH_TOUCH_PARA_FLAG_VALUE && forceCal == 1)
		{
			W25Q128_Read((uint8_t *)&strXPT2046_TouchPara, FLASH_TOUCH_PARA_ADDR, 4*6*8);	
		}
		...
		//写入触摸参数标志
		W25Q128_Write_Page(&para_flag,FLASH_TOUCH_PARA_FLAG_ADDR,1);
		//写入最新的触摸参数
		W25Q128_Write_Page((uint8_t *)&strXPT2046_TouchPara,FLASH_TOUCH_PARA_ADDR,4*6*8);
...
void XPT2046_TouchEvenHandler(void)
{
      ...
      uint8_t result = XPT2046_TouchDetect();
        printf("detectResult = %d\r\n",result);
		if(result == TOUCH_PRESSED)
		{
			led_green_toggle();

...
{ 
  ...
  else if(res_flash!=FR_OK)
  {
    ...
    printf("！！可能原因：sdio sdcard 初始化不成功。\r\n");
		while(1);
  }
  else
  {
	...
	/*------------------- 文件系统测试：写测试 --------------------------*/
	printf("\r\n****** 即将进行文件写入测试... ******\r\n");	
	res_flash = f_open(&fnew, "1:新建文本文档abc.txt",FA_CREATE_ALWAYS | FA_WRITE);
	if(res_flash == FR_OK )
	{
  ...
  /*------------------- 文件系统测试：读测试 --------------------------*/
	printf("****** 即将进行文件读取测试... ******\r\n");
	res_flash = f_open(&fnew, "1:新建文本文档abc.txt",FA_OPEN_EXISTING | FA_READ); 	 
	if(res_flash == FR_OK)
	{
		LED_GRREN = 0;
		printf("> 打开文件成功。\r\n");
...
{  
  /* 获取文件信息 */
  res_flash=f_stat("1:新建文本文档abc.txt",&fno);
	 printf("res_sdcard = %d\r\n",res_flash);
  if(res_flash==FR_OK)
  {

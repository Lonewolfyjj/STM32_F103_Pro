...
//	unsigned char priority = 0x7;
    ...
    GPIOE_CRL &= ~(0XF << 8);
	GPIOE_CRL |= 0x8 << 8;// 设置PE2 上下拉输入模式 2MHZ
    GPIOE->ODR|=1<<2;	   	//PE2 上拉
	
    
    GPIOE_CRL &= ~(0XF << 4);
	GPIOE_CRL |= 0x8 << 4;// 设置PE1 上下拉输入模式 2MHZ
...
unsigned int key_scan(void) 
{
  static unsigned char key_flag = 1; // 按键状态 1表示叹谈起，0表示按下 
	if ((key_flag == 1) && (KEY0 == 0 || KEY1== 0 || KEY2 == 0 || KEY3 == 0)) {
		delay_ms(10);
	...
	if ((key_flag == 0) && KEY0 == 1 && KEY1== 1 && KEY2 == 1 && KEY3 == 1) {
		delay_ms(10);
